...
async function main() {
  ...
  // Find the source clinic with pricing (Austin Central)
  const sourceClinic = await prisma.clinic.findFirst({
    where: { name: { contains: 'Emancipet - Austin Central' } },
    include: {
  ...
  if (!sourceClinic) {
    console.log('Source clinic (Emancipet - Austin Central) not found');
    return;
  }
